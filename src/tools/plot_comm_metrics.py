...
def _get_display_name(df):
    if "public_name" in df.columns:
        return df["public_name"].astype(str).apply(public_label)
    if "run_name" in df.columns:
        return df["run_name"].astype(str).apply(public_label)
    if "run" in df.columns:
        return df["run"].astype(str).apply(public_label)
    if "strategy" in df.columns:
        return df["strategy"].astype(str)
    return pd.Series(["run"] * len(df))


def _annotate_points(df, x, y):
    labels = _get_display_name(df)
    for _, row in df.iterrows():
        lbl = public_label(str(row.get("public_name", row.get("run_name", row.get("run", row.get("strategy", "run"))))))
        plt.annotate(lbl, (row[x], row[y]), fontsize=8, alpha=0.8)


def main():
    logger = get_logger("PlotCommMetrics")
    parser = argparse.ArgumentParser()
    parser.add_argument("--csv", type=str, required=True, help="Path to comm_metrics_epoch.csv or clean_summary.csv")
    ...
    _annotate_points(df_inf, "feature_normalized_ratio", "ap_70")
    ...
    _annotate_points(df_inf, "total_normalized_ratio", "ap_70")
    ...
    _annotate_points(df_inf, "total_bytes_per_frame", "ap_70")
    ...
    _annotate_points(df_inf, "total_normalized_ratio", "ap_50")
    plt.xlabel("Total Communication Ratio (normalized)")
    plt.ylabel("AP@0.5")
    ...
    if "packet_loss_rate" in df_inf.columns:
        ...
        _annotate_points(df_inf, "packet_loss_rate", "ap_70")
    ...
    _annotate_points(df_inf, "active_neighbors_ratio", "total_bytes_per_frame")
    ...
    if "context_bytes_per_frame" in df_inf.columns:
        group_key = "public_name" if "public_name" in df_inf.columns else ("strategy" if "strategy" in df_inf.columns else None)
        if group_key is not None:
            by_method = df_inf.groupby(group_key, as_index=False)["context_bytes_per_frame"].mean()
            by_method["label"] = by_method[group_key].astype(str).apply(public_label)
            plt.figure(figsize=(10, 5))
            plt.bar(by_method["label"], by_method["context_bytes_per_frame"])
