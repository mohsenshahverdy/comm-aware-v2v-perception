...
### 4.6 Academic Paper Result Anchors on OPV2V / Related Cooperative-Perception Datasets

These numbers are **not all directly comparable** to our results because papers differ in detector backbone, sensor modality, test range, compression accounting, and whether they use camera-only or LiDAR. The purpose is to show how each research family is evaluated in the literature and how our experiments relate to it.

| Research family | Representative paper / dataset | Sample reported result | How it relates to our project |
|---|---|---|---|
| Dense / intermediate fusion benchmark | **OPV2V** benchmark on Default CARLA + Culver City | With **PointPillar intermediate fusion**, OPV2V reports **Default CARLA AP@0.5/0.7 = 0.908 / 0.815** and **Culver AP@0.5/0.7 = 0.854 / 0.735**. | This is the closest academic baseline family to our PointPillars + intermediate-fusion setup. Our Phase 0/1 reproduces a similar full-communication evaluation idea, but with our checkpoint/config. |
| Importance-based spatial selection | **Where2comm** on OPV2V | Where2comm reports OPV2V camera-only AP@0.5/0.7 examples such as **44.07 / 17.15** at communication volume **17.04**, and **47.14 / 19.07** at communication volume **22.71**. It also reports that it outperforms methods such as V2X-ViT and DiscoNet on the OPV2V trade-off curve. | This is the most conceptually aligned category with our Phase 2 top-k energy and Phase 3 learnable mask. Our top-k result is simpler than Where2comm, but it supports the same idea: spatial importance matters. |
| Agent scheduling / communication-graph selection | **When2com / Where2comm comparison** on OPV2V | In Where2comm's OPV2V table, **When2com** is reported at communication volume **22.28** with **AP@0.5/0.7 = 19.69 / 8.29**, while Where2comm improves the OPV2V trade-off at several lower/higher budgets. | This motivates our neighbor-selection/active-neighbor metrics, but our current strongest result is spatial-cell selection, not full scheduling. |
| Compression / codebook representation | **CodeFilling** on DAIR-V2X and OPV2VH+ | CodeFilling replaces dense float feature vectors with code indices and reports that it outperforms Where2comm with **1,333× lower communication on DAIR-V2X** and **1,206× lower communication on OPV2VH+**. | This is a future direction after our top-k/learnable selection: first decide **where** to send, then compress **what** is selected using codebooks or quantization. |
| Lossy communication / repair | **V2VAM + LCRN** on OPV2V CARLA + Culver City | Under lossy communication, V2VAM+LCRN reports **CARLA AP@0.5/0.7 = 0.841 / 0.705** and **Culver AP@0.5/0.7 = 0.846 / 0.663**. | This directly motivates Phase 4. But based on our results, Phase 4 should be tested under a strict top-k or packet-loss bottleneck, not after a high-communication Phase 3 mask. |
| Alternative sparse message units | **Which2comm** on V2XSet and OPV2V | Which2comm proposes transmitting object-level sparse features / semantic detection boxes and reports that it outperforms SOTA on both perception performance and communication cost on V2XSet and OPV2V. | This is more invasive than our BEV-cell masking, but useful as future work if we move from feature-cell communication to object/token-level communication. |

**Meeting interpretation:** The academic literature supports the same direction as our experiments: dense fusion gives accuracy, but communication-efficient methods need either (i) spatial importance, (ii) compact representation, (iii) agent scheduling, or (iv) repair under bad links. Our Phase 2 result is strongest because it provides a clean, reproducible importance-based communication baseline on both CARLA and Culver.
...
## 18. Recommended Next Technical Step
...
### External academic sources used for the related-work result anchors

- OPV2V: *An Open Benchmark Dataset and Fusion Pipeline for Perception with Vehicle-to-Vehicle Communication*.
- Where2comm: *Communication-Efficient Collaborative Perception via Spatial Confidence Maps*.
- CodeFilling: *Communication-Efficient Collaborative Perception via Information Filling with Codebook*.
- V2VAM/LCRN: *Learning for Vehicle-to-Vehicle Cooperative Perception under Lossy Communication*.
- Which2comm: *An Efficient Collaborative Perception Framework for 3D Object Detection*.


## 20. Appendix — Short Tables for Quick Discussion
